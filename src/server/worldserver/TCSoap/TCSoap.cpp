...
void TCSoapRunnable::run()
{
    ...
    if (m < 0)
    {
        sLog.outError("CeSoap: couldn't bind to %s:%d", m_host.c_str(), m_port);
        exit(-1);
    }

    sLog.outString("CeSoap: bound to http://%s:%d", m_host.c_str(), m_port);
    ...
    while(!World::IsStopped())
    {
        ...
        sLog.outDebug("CeSoap: accepted connection from IP=%d.%d.%d.%d", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        sLog.outDebug("CeSoap: Client didn't provide login information");
        return 401;
    }
    ...
    if(!accountId)
    {
        sLog.outDebug("CeSoap: Client used invalid username '%s'", soap->userid);
        return 401;
    }

    if(!sAccountMgr.CheckPassword(accountId, soap->passwd))
    {
        sLog.outDebug("CeSoap: invalid password for account '%s'", soap->userid);
        return 401;
    }

    if(sAccountMgr.GetSecurity(accountId) < SEC_ADMINISTRATOR)
    {
        sLog.outDebug("CeSoap: %s's gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    sLog.outDebug("CeSoap: got command '%s'", command);
    ...
    if(acc)
    {
        sLog.outError("CeSoap: Error while acquiring lock, acc = %i, errno = %u", acc, errno);
    }
